...
program principal_melhorado
  ...
  ! ------------------- agora alocáveis --------------------
  real(8), allocatable           :: eig(:), eig_varredura(:)
  type(info_autovalor), allocatable :: info_detalhada(:)

  integer :: i
  real(8) :: tempo_inicio, tempo_fim
  
  !---------------------------------------------------------
  ! Aloca os vetores com base em n_eigen
  !---------------------------------------------------------
  allocate(eig(n_eigen), eig_varredura(n_eigen), info_detalhada(n_eigen))
  eig            = 0.0d0
  eig_varredura  = 0.0d0
  
  print '(A,I2)', ' Nível de verbosidade: ', verbose_level
  
  ! Método 1: Com validação de autovalores 
  ...
  !---------------------------------------------------------
  deallocate(eig, eig_varredura, info_detalhada)
